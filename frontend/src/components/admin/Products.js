...
function Products() {
    ...
    const deleteProduct = async(_id) => {
        var data = await Constants.DeleteAPI(`deleteproduct/${_id}`, null)
        if(data){
            fetchData();
                        ...
                        <div className="card mb-4">
                            ...
                            <div className="card-body">
                                <table className="table table-bordered">
                                    ...
                                    <tbody>
                                    {getProductArray?.length && getProductArray.map(share =>{
                                      return(
                                        <tr key={share._id}>
                                        ...
                                        <div className="modal fade" id={'myModal-'+share._id} role="dialog">
                                            ...
                                            <div className="modal-content">
                                                ...
                                                <button onClick={()=>deleteProduct(share._id)} type="button" className="btn btn-success yess" data-dismiss="modal">Yes</button>
                                            ...
                                            <td>{share._id}</td>
                                            <td>{share.name}</td>
                                            <td>{share.category.name}</td>
                                            <td>{share.price}</td>
                                            <td><img src={share.image} width="100px" height="100px" /></td>
                                            <td>{share.countInStock}</td>
                                            <td><Link to={`/admin/editproduct/${share._id}`} className="nav-link">Edit</Link></td>
